    ...
    try {
      if (listaOpciones[0].length >= 1 || listaOpciones.length >= 1) {
        return listaOpciones.map((tupla) => (
          <option key={uuidv4()} defaultValue={tupla[0]["valor"]}>
            {tupla[0]["valor"]}
          </option>
        ));
      } else {
        return (
          <option key={uuidv4()} defaultValue="no hay datos">
            no hay datos
          </option>
        );
      }
      ...
      <div className="selectBase">
        ...
        <label
          className={
            "selectBase__texto  " +
            (datosValido
              ? "selectBase__texto--verde"
              : "selectBase__texto--rojo") +
            (valueCampo ? "  selectBase__texto--vacio" : "  ")
          }
        >
          {texto}
        </label>
