...
import { precioBase, precioVenta } from "../../../../../backend/funcioneGenerales.js";
		...
		classEmple.productoBase()
			.then(resultado => {
				...
				let productosFiltrados = resultado.filter((tupla, index) => {
					...
					) === index && (tupla["PRODUCTO_DISPONIBLE"] == 1 && tupla["CATEGORIA_DISPONIBLE"] == 1);
				});
	...
	const catalogoActivo = ((catalogoEstado) => {
		...
		if (catalogoEstado) {
			return <>
				...
				<div className={` ${carritoActivo ? " " : "carritoCatalogo--desactiva"}`}>
			...
			return <><PageNotAvaible></PageNotAvaible> </>
		}
	});
