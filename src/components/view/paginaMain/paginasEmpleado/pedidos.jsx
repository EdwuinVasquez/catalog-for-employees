	...
	const generarLista = async () => {
		classEmple.venta()
			.then(resultado => {
				...
				const misVentasLista = resultado.filter(tupla =>
					tupla["VENTA_CEDULA_USUARIO"] == contexUsuarioLogin["cedula"]
				);

				const misVentasListaFiltradas = misVentasLista.filter((tupla, index) => {
					return misVentasLista.indexOf(
						misVentasLista.find(valor => valor["VENTA_CODIGO_VENTA"] == tupla["VENTA_CODIGO_VENTA"])
					) === index;
				});
