...
export function EmpleadosLista() {
  ...
  const eliminarEmpleadoSistema = async (cedula, dato) => {
    alertaConfirmar(
      ...
      .then((confirm) => {
        if (confirm) {
          classAdmin.eliminarEmpleado(cedula).then(() => {
            classAdmin.usuarios(cedula).then((resultado) => {
              if (resultado.length == 0) {
                ...
                alertaToast(
                  "warning",
                  "El usuario no se pudo eliminado",
                  5000,
                  "top-end"
                );
              }
        ...
        if (confirm) {
          alertaInput()
            .then((resultado) => {
              ...
              classAdmin.cambiarClave(nuevosDatos).then((resultado) => {
                alertaToast(
                  "success",
                  `${resultado[0]["RESULTADO"]}`,
                  5000,
