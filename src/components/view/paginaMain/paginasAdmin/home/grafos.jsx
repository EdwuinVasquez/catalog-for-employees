...
export const HomeGraficos = () => {
        ...
        if (ventas.length > 0) {
          ...
          ventas.forEach((venta) => {
            ...
            if (ventasPorEstadoFecha[estado][key]) {
              ventasPorEstadoFecha[estado][key] += parseInt(
                venta.VENTA_VALOR_TOTAL
              );
            } else {
              ventasPorEstadoFecha[estado][key] = parseInt(
                venta.VENTA_VALOR_TOTAL
          ...
          ventas.forEach((venta) => {
            ...
            if (ventasPorEmpleado[estado][key]) {
              ventasPorEmpleado[estado][key] += parseInt(
                venta.VENTA_VALOR_TOTAL
              );
            } else {
              ventasPorEmpleado[estado][key] = parseInt(
          ...
          productoVenta.forEach((venta) => {
            ...
            if (ventasPorProducto[estado][key]) {
              ventasPorProducto[estado][key] += parseInt(
                venta.VENTA_VALOR_TOTAL
              );
            } else {
              ventasPorProducto[estado][key] = parseInt(
  ...
  return (
    <>
      ...
      ></FiltrosDatosTabla>
      <canvas ref={chartRef} id="salesChart" width="800" height="400"></canvas>{" "}
      <br />
      <canvas
        ref={chartRefOne}
        id="salesChartOne"
