...
export function ProductoCRUD() {
	...
	const generarLista = async () => {
		...
		classAdmin.productoBase()
			.then(resultado => {
				...
				alertaToast("success", "Los datos fueron cargados correctamente", 5000, "top-end");
			...
			<datalist id="lista__productos">
				{
					productosFiltrados.map((value) => {
						return <option value={value["CODIGO_PRODUCTO"]}>{value["NOMBRE_PRODUCTO"]}</option>
					})
				}
			</datalist>
		</>
	...
	const listaCategorias = ((lista) => {
		if (lista.length > 0 && lista != false) {
			return <>
				{
					lista.map((value) => {
						return <option value={value["CODIGO_CATEGORIA"]}>{value["NOMBRE_CATEGORIA"]}</option>
					})
				}
			</>
		}
	...
	const buscarProducto = ((e) => {
		...
		if (productosFiltrados.length > 0) {
			...
			alertaToast("success", "Datos almacenados cargados en pantalla", 5000, "top-end");
		} else {
			alertaToast("warning", "Error no se encontraron datos relacionados", 5000, "top-end");
		}
		setProductoListaCompleta(nuevosProductoCompleto)
		/*-- Asignar valor inputs --*/
		document.getElementById("productoNombreBase").value = nuevosDatos["NOMBRE_PRODUCTO"];
		document.getElementById("productoIvaBase").value = nuevosDatos["IVA"];
		document.getElementById("productoDescuentoBase").value = nuevosDatos["DESCUENTO"];
		document.getElementById("productoValorBase").value = nuevosDatos["PRECIO_BASE"];
		document.getElementById("productoEstadoBase").value = nuevosDatos["PRODUCTO_DISPONIBLE"];
		document.getElementById("productoImagenBase").value = nuevosDatos["IMAGEN_PRINCIPAL"];
		document.getElementById("productoCategoriaBase").value = nuevosDatos["ID_CATEGORIA"];
		document.getElementById("productoDetallesBase").value = nuevosDatos["DETALLES"];
	...
	const value = ((e, id) => {
		...
		if ((id == "IVA" || id == "PRECIO_BASE" || id == "DESCUENTO") && e.target.value.trim() == "") {
			nuevosDatos[id] = 0;
		}
		setDatosProductoBase(nuevosDatos);
	...
	const datosItemCambioBase = ((nuevaBase) => {
		if (productoListaCompleta != false) {
			const nuevosProductoCompleto = productoListaCompleta.map((valor) => {
				...
				datosActualizados[0]["productoDisponible"] = nuevaBase["PRODUCTO_DISPONIBLE"];
				...
				datosActualizados[0]["imgItem"] = imagenItem(nuevaBase["IMAGEN_PRINCIPAL"], valor[0]["imgExtra"]);
				...
				datosActualizados[0]["nombreItem"] = `${nuevaBase["NOMBRE_PRODUCTO"]} - ${valor[0]["estilo"]}`;
				datosActualizados[0]["valor"] = precioVenta(nuevaBase["PRECIO_BASE"], nuevaBase["IVA"], nuevaBase["DESCUENTO"]);
				datosActualizados[0]["valorBase"] = precioBase(nuevaBase["PRECIO_BASE"], nuevaBase["IVA"]);
	...
	const datosDetallados = (() => {
		try {
			if (productoListaCompleta.length >= 1) {
				...
				alertaToast("warning", "No hay información adicional del ítem", 5000, "top-end");
			}
		} catch (error) {
			alertaToast("warning", "Error al obtener los datos", 5000, "top-end");
		}
	...
	const eliminarItem = (id, parametro) => {
		...
		alertaToast("warning", `${id.toUpperCase()} fue eliminado`, 5000, "top-end");
	...
	const guardarProducto = (() => {
		if (productoListaCompleta != false) {
			classAdmin.finalizarActualizacionProducto(productoListaCompleta)
				.then((resultado) => {
					if (resultado) {
						alertaToast("success", "Datos almacenados", 5000, "top-end");
					} else {
						alertaToast("error", "Error al guardar la información", 5000, "top-end");
					}
				}).catch(error => {
					alertaToast("error", "Error al guardar la información", 5000, "top-end");
				})
	...
	const agregarProductoEstilo = ((estilo) => {
		const nuevaTupla = {
			...
			valor: precioVenta(datosProductoBase["PRECIO_BASE"], datosProductoBase["IVA"], datosProductoBase["DESCUENTO"]),
			valorBase: precioBase(datosProductoBase["PRECIO_BASE"], datosProductoBase["IVA"]),
			...
			imgItem: imagenItem(datosProductoBase["IMAGEN_PRINCIPAL"], estilo["imagenExtraInfo"]),
			imgExtra: estilo["imagenExtraInfo"],
			disponible: estilo["estadoInfo"],
			stock: "0",
			tipoContenido: estilo["tipoContenidoInfo"],
			contenido: (estilo["tipoContenidoInfo"] == 0 ? estilo["contenidoHexaInfo"] : estilo["contenidoImgInfo"]),
			informacionAdicional: "",
			estadoDetalles: detallesActivo
		}
		if (productoListaCompleta != false) {
			const verificarEstilo = productoListaCompleta.filter((tupla) => {
				return (tupla[0]["estilo"].toUpperCase() == estilo["estiloInfo"].toUpperCase());
			});
			if (verificarEstilo.length > 0) {
				alertaToast("error", "Ya hay un producto vinculado con ese nombre", 5000, "top-end");
			} else {
		...
		<div style={{ width: "100%", padding: "10px" }}>
			...
			<div style={{
				...
				<div className="campoVerificado">
					<input style={{ width: "96%" }} list='lista__productos' className="campoVerificado__campo" onChange={buscarProducto} type="number" required="" autoComplete="off" />
				...
				<Tooltip title="Previsualizar">
					<div style={{
						...
						<FaEye onClick={abrirDestalles} style={{
							width: "30px",
							height: "30px",
							cursor: "pointer",
						}}></FaEye>
			...
			{/*-- Nombre --*/}
			<div className="campoVerificado">
				<input id='productoNombreBase' className="campoVerificado__campo" onChange={(e) => value(e, "NOMBRE_PRODUCTO")} type="text" required="" autoComplete="off" />
				<label className={"campoVerificado__texto"}>Nombre producto</label>
			</div>
			{/*-- Estado --*/}
			<div className="selectBase" style={{ width: "92%" }}>
				<select id='productoEstadoBase' className="selectBase__select" onChange={(e) => value(e, "PRODUCTO_DISPONIBLE")} required="" autoComplete="off">
			...
			<div style={{
				...
				<div className="selectBase" style={{ width: "98%" }}>
					<select id='productoImagenBase' className="selectBase__select" onChange={(e) => value(e, "IMAGEN_PRINCIPAL")} required="" autoComplete="off">
						<option selected={true} defaultValue="NAN" disabled="disabled">seleccione una opcion</option>
			...
			<div style={{
				...
				<div className="selectBase" style={{ width: "98%" }}>
					<select id='productoCategoriaBase' className="selectBase__select" onChange={(e) => value(e, "ID_CATEGORIA")} required="" autoComplete="off">
						<option selected={true} defaultValue="NAN" disabled="disabled">seleccione una opcion</option>
			...
			{/*-- Iva --*/}
			<div className="campoVerificado">
				<input id='productoIvaBase' className="campoVerificado__campo" onChange={(e) => value(e, "IVA")} type="number" min="0" step="any" required="" autoComplete="off" />
				<label className={"campoVerificado__texto"}>Iva del producto</label>
			</div>
			{/*-- Descuento --*/}
			<div className="campoVerificado">
				<input id='productoDescuentoBase' className="campoVerificado__campo" onChange={(e) => value(e, "DESCUENTO")} type="number" min="0" step="any" required="" autoComplete="off" />
				<label className={"campoVerificado__texto"}>Descuento del producto</label>
			</div>
			{/*-- Precio --*/}
			<div className="campoVerificado">
				<input id='productoValorBase' className="campoVerificado__campo" onChange={(e) => value(e, "PRECIO_BASE")} type="number" min="0" step="any" required="" autoComplete="off" />
				<label className={"campoVerificado__texto"}>Valor del producto</label>
			</div>
			{/*-- Destalles --*/}
			<div className="campoVerificado">
				<input id='productoDetallesBase' className="campoVerificado__campo" onChange={(e) => value(e, "DETALLES")} type="text" required="" autoComplete="off" />
				<label className={"campoVerificado__texto"}>Detalles producto</label>
			</div>
			<h1>lista de estilos</h1>
			<ListaItemProducto datosTabla={productoListaCompleta} cambiarEstado={cambiarEstadoItem} eliminarItem={eliminarItem} imagenes={datosImagenes} agregar={agregarProductoEstilo}></ListaItemProducto>
			<div className={`contenedor__detallesProducto  ${detallesActivo && productoListaCompleta != false ? "" : "contenedor__detallesProducto--desactiva"}`}>
				{datosDetallados()}
			</div>
			{/*-- BotonSencillo --*/}
			<div style={{ width: "92%" }}>
				<BotonSencillo texto="Guardar" manejarClik={guardarProducto}></BotonSencillo>
			</div>
