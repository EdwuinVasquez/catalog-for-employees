...
export function CategoriaLista() {
  ...
  const titulosTabla = [
    ...
    "Modificar iva",
    "Modificar descuento",
    "Modificar precio en valor",
    "Modificar pecio en porcentaje",
    "Eliminar",
  ];
        ...
        let productosFiltrados = resultado.filter((tupla, index) => {
          return (
            resultado.indexOf(
              resultado.find(
                (valor) =>
                  valor["CODIGO_CATEGORIA"] == tupla["CODIGO_CATEGORIA"]
              )
            ) === index
          );
        });
  ...
  const eliminarCategoria = async (codigo, dato) => {
    alertaConfirmar(
      ...
      .then((confirm) => {
        if (confirm) {
          classAdmin.eliminarCategoria(codigo).then((resultado) => {
            alertaToast("success", "Proceso finalizado", 5000, "top-end");
            generarLista();
          });
        }
      })
  ...
  const modificarEstadoCategoria = async (codigo, dato) => {
    ...
    classAdmin
      .modificarEstadoCategoria(nuevoEstado)
      .then((resultado) => {
        if (resultado === false) {
          console.clear();
          throw new Error("No hay resultados almacenados");
          ...
          alertaInput()
            .then((resultado) => {
              if (resultado == undefined || resultado == "")
                throw new Error("No se ingreso un valor valido");
              const nuevosDatos = {
                codigo: codigo,
                iva: resultado,
              };
              classAdmin
                .modificarIvaCategoria(nuevosDatos)
                .then((resultado) => {
                  alertaToast(
                    "success",
        ...
        if (confirm) {
          alertaInput()
            .then((resultado) => {
              ...
              classAdmin
                .modificarDescuentoCategoria(nuevosDatos)
                .then((resultado) => {
                  alertaToast(
                    "success",
                    `Categoría actualizada`,
        ...
        if (confirm) {
          alertaInput()
            .then((resultado) => {
              ...
              classAdmin
                .modificarValorNumeroCategoria(nuevosDatos)
                .then((resultado) => {
                  alertaToast(
                    "success",
                    `Categoría actualizada`,
        ...
        if (confirm) {
          alertaInput()
            .then((resultado) => {
              ...
              classAdmin
                .modificarValorPorcentajeCategoria(nuevosDatos)
                .then((resultado) => {
                  alertaToast(
                    "success",
                    `Categoría actualizada`,
        ...
        if (confirm) {
          alertaInput()
            .then((resultado) => {
              ...
              classAdmin
                .modificarNombreCategoria(nuevosDatos)
                .then((resultado) => {
