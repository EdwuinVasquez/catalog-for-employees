...
export function AjustesCompra() {
  ...
  const listaDeComprasSelet = () => {
    classAdmin
      ...
      .then((resultado) => {
        ...
        setDatosSelect(
          listaDatos.map((tupla) => {
            return {
              label: `${tupla["VENTA_CODIGO_VENTA"]} - ${tupla["USUARIO_NOMBRE"]}`,
            };
          })
        );
      })
  ...
  const obtenerDatosCompra = (compra) => {
    classAdmin
      ...
      .then((resultado) => {
        ...
        const DatosCompra = resultado.filter(
          (valor) =>
            `${valor["VENTA_CODIGO_VENTA"]} - ${valor["USUARIO_NOMBRE"]}`.toUpperCase() ==
            compra
        );
  ...
  const generarLista = async () => {
    try {
      if (
        !(cedulaEmpleado != undefined && cedulaEmpleado != "") &&
  ...
  return (
    <>
      <BuscadorSelect
        lista={datosSelect}
        manejarCambio={obtenerDatosCompra}
        manejarClick={generarLista}
      ></BuscadorSelect>
      <AgregarItem actualizar={generarLista}></AgregarItem>
      <TablaMain
        buscadorNombre="Codigo o nombre"
        buscadorTitulo={`Modificar compra ${
          codigoCompra == undefined ? "" : codigoCompra
        }`}
