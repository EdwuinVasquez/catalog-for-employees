...
export function ListaComprasAdmin() {
			...
			.then(resultado => {
				...
				const nuevosDatos = informacionVentaBase.map((valor) =>
					[{
						...
						parametro: { codigo: valor["VENTA_CODIGO_VENTA"], cedula: valor["VENTA_CEDULA_USUARIO"] }
					}, {
						key: valor["ITEM_NOMBRE_ITEM"],
						id: valor["ITEM_NOMBRE_ITEM"],
						tipo: "botonMas",
	...
	const datosDestallasdosVenta = (parametros) => {
		const venta = parametros["lista"].filter(tupla =>
			tupla["VENTA_CODIGO_VENTA"] == parametros["codigo"]
		);
	...
	const modificarEstadoVenta = (nuevosDatos) => {
		classAdmin.modificarEstadoVenta(nuevosDatos)
			.then((resultado) => {
				alertaToast("success",
					`Se cambió el estado de la venta N.º${resultado[0]["CODIGO_VENTA"]} a un estado ${nombreEstado(resultado[0]["ESTADO_VENTA"])}`,
					5000,
					"top-end");
				generarListaVenta();
			}).catch((error) => {
