...
/*-- Titulos de la tabla --*/
const titulosTabla = ["Pedido numero", "Fecha", "Item", "Cantidad", "Valor unitario", "Valor Total", "Estado", "Empleado"];
const header = ["Pedido numero", "Item", "Cantidad", "IVA", "Valor unitario", "Valor Total", "Estado", "Empleado"];
	...
	const generarLista = async () => {
		classEmple.venta()
			.then(resultado => {
				...
				// Calcular valor iva
				const valorIva = datosFiltrados.reduce((total, tupla) => evaluate(`${total}+(${tupla["ITEM_VALOR_TOTAL"]}*0.${tupla["ITEM_IVA"]})`), 0);

				// Calcular valor total
				const valorTotal = datosFiltrados.reduce((total, tupla) => evaluate(`${total}+${tupla["ITEM_VALOR_TOTAL"]}`), 0);

				// Calcular porcentaje iva
				const sumaIva = datosFiltrados.reduce((total, tupla) => evaluate(`${total}+${tupla["ITEM_IVA"]}`), 0);
				...
				const nuevoContenidoTablaExcel = datosFiltrados.map((valor) =>
					...
					valor["ITEM_CANTIDAD"],
					evaluate(`(${valor["ITEM_VALOR_TOTAL"]}*0.${valor["ITEM_IVA"]})`).toFixed(0),
				...
				nuevoContenidoTablaExcel.push(
					["Sin iva", "", "", "", "", "", "", valorNeto.toFixed(0)],
					["Iva", "", "", "", "", "", porcentajeIva.toFixed(0), valorIva.toFixed(0)],
				...
				const nuevoContenidoTablaCsv = datosFiltrados.map((valor) => {
					return {
						...
						Valor_iva: evaluate(`(${valor["ITEM_VALOR_TOTAL"]}*0.${valor["ITEM_IVA"]})`).toFixed(0),
						Valor_unitario: valor["ITEM_VALOR_DESCUENTO"],
						Valor_total: valor["ITEM_VALOR_TOTAL"],
						Estado_compra: asignarEstadoNombre(parseInt(valor["VENTA_ESTADO_VENTA"])),
						Empleado: valor["USUARIO_NOMBRE"]
					}
				});
				nuevoContenidoTablaCsv.push(
					{ Numero_pedido: "Neto", Producto: "", Cantidad: "", Valor_iva: "", Valor_unitario: "", Valor_total: "", Estado_compra: "", Empleado: valorNeto.toFixed(0) },
					{ Numero_pedido: "Iva", Producto: "", Cantidad: "", Valor_iva: "", Valor_unitario: "", Valor_total: "", Estado_compra: "", Empleado: valorIva.toFixed(0) },
					{ Numero_pedido: "Total", Producto: "", Cantidad: "", Valor_iva: "", Valor_unitario: "", Valor_total: "", Estado_compra: "", Empleado: valorTotal.toFixed(0) });
