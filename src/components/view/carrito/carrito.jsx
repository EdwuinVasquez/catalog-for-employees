...
import { cerrarAlertaAction as action, formatearNumero, imprimir } from '../../../backend/funcioneGenerales';
...
export function CarritoCatalogo({ manejarClick }) {
	...
	const calcularCosto = (() => {
		const total = carrito.reduce((valorAnterior, valorActual) => {
			return valorAnterior + (valorActual[0]["cantidad"] * valorActual[0]["valor"]);
		}, 0);
	...
	return (
		<><div className="carritoCatalogo">
			<div className="carritoCatalogo__tituloSuperios">
				<span className="carritoCatalogo__tituloSuperios--titulo">Carrito de compra</span>
			...
			<div className='carritoCatalogo__valor'>
				<section className='carritoCatalogo__valor--section'>
					...
					<span className='carritoCatalogo__valor--precio'>${calcularCosto()}</span>
				</section>
				<section className='carritoCatalogo__valor--section'>
					<Link to="carrito">
						<button className="carritoCatalogo__boton  carritoCatalogo__boton--gris">Ver carrito</button>
					</Link>
				</section>
				<section className='carritoCatalogo__valor--section'>
					<button
