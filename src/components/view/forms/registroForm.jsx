...
import { listaOpciones } from "../../../backend/funcioneGenerales.jsx";
...
export function RegistroFomulario() {
  ...
  const actualizarDatos = (dato, correcto, key) => {
    setDatos(() => {
      ...
      if (key == "nombres" || key == "apellidos")
        nuevosDatos[
          "nombre"
        ] = `${nuevosDatos["nombres"]} ${nuevosDatos["apellidos"]}`;
      return nuevosDatos;
    });
  ...
  const registarse = async (e) => {
    ...
    if (todosLosDatosSonCorrectos()) {
        ...
        .then((resultado) => {
          ...
          alertaToast(
            ...
            resultado[0]["RESULTADO"].toUpperCase().includes("ERROR")
              ...
              : `${resultado[0]["RESULTADO"]}, COMUNIQUESE CON UN ADMINISTRADOR PARA SU VERIFICACION`,
            5000,
            "top-end"
          );
          if (
      ...
      alertaToast(
        "warning",
        "Aún hay datos sin llenar o incorrectos",
        5000,
        "top-end"
      );
    }
    ...
    <>
      ...
      >
        <form className="registroFomulario">
          ...
          <BotonSencillo
            texto="Registrarse"
            manejarClik={registarse}
          ></BotonSencillo>
          <Link to="/login">
            <BotonSencillo
              texto="Ingresar"
              manejarClik={() => console.log()}
            ></BotonSencillo>
          </Link>
        </form>
      </div>
